...
if [[ ${PV} == *9999* ]]; then
	...
	SRC_URI="http://github.com/uzbl/${PN}/tarball/${PV} -> ${P}.tar.gz"
fi
...
src_compile() {
	...
	emake PREFIX="${PREFIX}" ENABLE_WEBKIT2=yes ${gtk_var}=$(use gtk3 && echo 1 || echo 0)
}
...
src_install() {
	...
	# -j1 : upstream bug #351
	emake -j1 DESTDIR="${D}" ENABLE_WEBKIT2=yes PREFIX="${PREFIX}"	\
